...
#include "include/media.h"  // For SPIFFS mutex protection
#include <dirent.h>
#include <sys/stat.h>
#include <string.h>
...
static esp_err_t index_handler(httpd_req_t *req)
{
    ...
    // Lock SPIFFS for directory reading
    ...
    snprintf(nav_html, sizeof(nav_html),
             ...
             "• <strong>Hand gesture: ok</strong> - Take Photo<br>"
             "• <strong>Hand gesture: one</strong> - Start 10s Video Recording"
             "</p>",
...
static esp_err_t images_handler(httpd_req_t *req)
{
    ...
    // Lock SPIFFS for directory reading
    if (!media_lock_spiffs(2000)) {
        httpd_resp_sendstr_chunk(req, "<div class='empty'>Storage busy, try again later</div>");
        httpd_resp_sendstr_chunk(req, "</div>");
        httpd_resp_sendstr_chunk(req, html_footer);
        httpd_resp_sendstr_chunk(req, NULL);
        return ESP_OK;
    ...
    media_unlock_spiffs();  // Unlock after directory read

    if (count == 0) {
        httpd_resp_sendstr_chunk(req, "<div class='empty'>No photos found. Use hand gesture 'ok' to take a photo!</div>");
    }
...
static esp_err_t videos_handler(httpd_req_t *req)
{
    ...
    if (count == 0) {
        httpd_resp_sendstr_chunk(req, "<div class='empty'>No videos found. Use hand gesture 'one' to record a video!</div>");
    }
...
static esp_err_t download_handler(httpd_req_t *req)
{
    ...
    // Lock SPIFFS for thread-safe file read
    if (!media_lock_spiffs(5000)) {
        ESP_LOGW(TAG, "Failed to lock SPIFFS for download (timeout)");
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Storage busy, try again");
        return ESP_FAIL;
    }
    ...
    fclose(f);
    media_unlock_spiffs();  // Unlock after file read complete
