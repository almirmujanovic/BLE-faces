...
// -----------------------------------------------------------------------------
// SPIFFS mutex for thread-safe file access
// -----------------------------------------------------------------------------
static SemaphoreHandle_t s_spiffs_mutex = NULL;

bool media_lock_spiffs(uint32_t timeout_ms)
{
    if (!s_spiffs_mutex) {
        return false;  // Not initialized yet
    }
    return xSemaphoreTake(s_spiffs_mutex, pdMS_TO_TICKS(timeout_ms)) == pdTRUE;
}

void media_unlock_spiffs(void)
{
    if (s_spiffs_mutex) {
...
static size_t jpeg_encode_output(void *arg, size_t index, const void *data, size_t len)
{
    ...
    if (!ctx->buffer) {
        ...
        ctx->buffer   = (uint8_t *)heap_caps_malloc(ctx->capacity, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!ctx->buffer) {
    ...
    if (ctx->len + len > ctx->capacity) {
        ...
        uint8_t *new_buffer = (uint8_t *)heap_caps_realloc(ctx->buffer, new_capacity, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!new_buffer) {
            ESP_LOGE(TAG, "Failed to realloc JPEG buffer in PSRAM");
            return 0;
        }
...
esp_err_t media_storage_init(void)
{
    ...
    // Create SPIFFS mutex for thread-safe file access
    if (!s_spiffs_mutex) {
        ...
        ESP_LOGI(TAG, "SPIFFS mutex created for thread-safe access");
    }
...
static esp_err_t save_fb_as_jpeg_to_path(camera_fb_t *fb, const char *path)
{
    ...
    // Log memory BEFORE photo save
    ESP_LOGI(TAG, "📸 BEFORE photo - Internal free: %u, largest block: %u",
    ...
    // Lock SPIFFS for thread-safe file write
    if (!media_lock_spiffs(5000)) {
        ESP_LOGE(TAG, "Failed to lock SPIFFS mutex (timeout)");
        leds_set_capture(false);
    ...
    // Log memory AFTER photo save
    ESP_LOGI(TAG, "📸 AFTER photo - Internal free: %u, largest block: %u",
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
             (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL));
                                   ...
                                   size_t max_frames)
{
    ...
    // Lock SPIFFS for thread-safe file write (video capture)
    if (!media_lock_spiffs(5000)) {
        ESP_LOGE(TAG, "Failed to lock SPIFFS mutex for video (timeout)");
        return ESP_FAIL;
    }
    ...
    if (first_fb->format == PIXFORMAT_JPEG) {
        ...
        if (!encode_to_jpeg_quality(first_fb, VIDEO_JPEG_QUALITY, &first_jpeg, &first_jpeg_len)) {
            ...
            media_set_camera_busy(false);
            leds_set(false, false);
            ESP_LOGE(TAG, "Failed to encode first frame for video");
            return ESP_FAIL;
        }
    }
    ...
    fclose(f);
    media_unlock_spiffs();  // Unlock SPIFFS after video file is written
